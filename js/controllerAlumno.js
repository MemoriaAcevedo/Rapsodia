...
app.controller("alumnoPruebaCtrl", function($rootScope, $scope, $location, $http, restFactory, $mdDialog, viewFactory){
	...
	$scope.verificador = function(answer){
		for(var i = 0 ; i < answer.length ; i++){
			if(answer[i].respuesta == ""){
				return false;
			}
		}
		return true;
	...
	$scope.corregir = function(){
		...
		var confirm = $mdDialog.confirm()
	          ...
	          $mdDialog.show(confirm).then(function() {
						...
						var res = $scope.verificador($scope.test.respuestas);
						...
						if(res){
							...
							$scope.showAlert("Debe responder todas las preguntas de la prueba");
						}
...
/*Sprint 4*/
app.controller("materialTeoricoCtrl", function($rootScope, $scope, $location, $http, restFactory, $mdDialog, viewFactory, $anchorScroll){
